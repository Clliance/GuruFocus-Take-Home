...
export function* login(username, password) {
    ...
    try {
        ...
        yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Username or password incorrect',msgType:0});
    } finally {
...
export function* loginFlow() {
    while (true) {
        ...
        if(response&&response.code === 0){
            yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Sign in successful',msgType:1});
...
export function* registerFlow () {
    while(true){
        ...
        if(response&&response.code === 0){
            yield put({type:IndexActionTypes.SET_MESSAGE,msgContent:'Sign up successful',msgType:1});
            yield put({type:IndexActionTypes.RESPONSE_USER_INFO,data:response.data})
        }
